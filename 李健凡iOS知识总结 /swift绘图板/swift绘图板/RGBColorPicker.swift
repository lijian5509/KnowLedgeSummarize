...
typealias ColorChangedBlock = (color: UIColor) ->Void

class RGBColorPicker: UIView {

//    var colorChangedBlcok: ((color: UIColor) -> Void)?
    var colorChangedBlcok: ColorChangedBlock?
    ...
    @IBAction private func colorChanged(slide: UISlider) {
        ...
        let label = self.labels[self.sliders.indexOf(slide)!]
        
        label.text = NSString(format: "%.0f", slide.value) as String
        
        if let colorChangedBlcok = self.colorChangedBlcok {
            colorChangedBlcok(color: color)
        }
    }
